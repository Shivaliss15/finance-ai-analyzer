...
# Path to the precomputed LSTM data file (committed to GitHub)
LSTM_PKL_PATH = "lstm_results.pkl"
...
def create_features(df):
    ...
    df['rolling_7_net']  = df.groupby('Account No')['net_amount'].transform(lambda x: x.rolling(7,  min_periods=1).sum())
    df['rolling_30_net'] = df.groupby('Account No')['net_amount'].transform(lambda x: x.rolling(30, min_periods=1).sum())
...
def run_transaction_categorization(df):
    ...
    def auto_label(text):
        text = str(text).lower()
        if 'chq' in text:                                          return 'cheque'
        elif 'rtgs' in text:                                       return 'rtgs'
        elif any(k in text for k in ['fund trf','transfer','trf']): return 'transfer'
        elif any(k in text for k in ['cash','atm','csh']):         return 'cash'
        elif any(k in text for k in ['charges','stax','commission']): return 'bank_charges'
        elif 'dep' in text:                                        return 'deposit'
        else:                                                      return 'other'
    ...
    num_cols = ['WITHDRAWAL AMT','DEPOSIT AMT','net_amount','month','weekday','is_weekend','rolling_7_net','rolling_30_net']
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y_enc, test_size=0.35, random_state=42, stratify=y_enc)
    X_train_text, X_test_text, y_train_text, y_test_text = train_test_split(X_text_vec, y_enc, test_size=0.35, random_state=42, stratify=y_enc)
    ...
    for name, model in models.items():
        ...
        results.append({
            "Model":    name,
            "Accuracy": round(accuracy_score(cur_y, preds), 4),
            "Recall":   round(recall_score(cur_y, preds, average='weighted', zero_division=0), 4),
            "Prec.":    round(precision_score(cur_y, preds, average='weighted', zero_division=0), 4),
            "F1":       round(f1_score(cur_y, preds, average='weighted', zero_division=0), 4),
...
def plot_transaction_category_distribution(df):
    cat_col = 'txn_category_pred' if 'txn_category_pred' in df.columns else 'txn_category_weak'
    category_counts   = df[cat_col].dropna().value_counts()
    category_percent  = (category_counts / category_counts.sum()) * 100
    large_cats = category_percent[category_percent >= 1].copy().sort_values(ascending=False)
    small_cats = category_percent[category_percent < 1]
    if len(small_cats) > 0:
...
def run_prophet_forecast(monthly):
    ...
    for acc in monthly['Account No'].unique():
        ...
        plt.fill_between(forecast['ds'], forecast['yhat_lower'], forecast['yhat_upper'], color='lightblue', alpha=0.3, label='Uncertainty')
        plt.title(f"Net Savings Forecast - Account {acc}")
        plt.xlabel("Month"); plt.ylabel("Net Savings")
        plt.legend(); plt.grid()
...
def get_prophet_evaluation(monthly):
    ...
    for acc in monthly['Account No'].unique():
        ...
        if len(prophet_df) < 12:
            continue
        q1, q3 = prophet_df['y'].quantile(0.25), prophet_df['y'].quantile(0.75)
        prophet_df['y'] = np.clip(prophet_df['y'], q1 - 1.5*(q3-q1), q3 + 1.5*(q3-q1))
        ...
        actual, predicted = test['y'].values, forecast_test['yhat'].values
        mae = mean_absolute_error(actual, predicted)
        rmse = np.sqrt(mean_squared_error(actual, predicted))
        prophet_metrics[acc] = {"MAE": mae, "RMSE": rmse, "Error Ratio": mae / (np.mean(np.abs(actual)) + 1)}
...
# =========================
# REBUILD LSTM FIGURES FROM DATA
# (used when loading from pkl — avoids pickling matplotlib objects)
# =========================
def _rebuild_lstm_figs(lstm_eval_data, monthly):
    """Recreate matplotlib figures from raw eval data stored in pkl."""
    lstm_figs = {}
    for acc, data in lstm_eval_data.items():
        try:
            user_df = (monthly[monthly['Account No'] == acc]
                       .copy().sort_values('ds')
                       .set_index('ds').resample('MS')['monthly_net']
                       .sum().fillna(0).reset_index()
                       .rename(columns={'monthly_net':'y'}))

            vals        = data["vals"]
            all_pred    = data["all_pred"]
            trim_idx    = data.get("trim_idx", 0)
            WINDOW      = data["window"]
            ...
            if len(future_dates) > 0 and len(future_pred) > 0:
                ...
                ax.axvline(user_df['ds'].max(), linestyle=':')
...
def run_lstm_forecast(monthly):
    ...
    # ── CLOUD: load precomputed results ──────────────────────
    if not TF_AVAILABLE:
        if os.path.exists(LSTM_PKL_PATH):
            with open(LSTM_PKL_PATH, "rb") as f:
                data = pickle.load(f)
            lstm_eval_data    = data["lstm_eval_data"]
            lstm_forecasts    = data["lstm_forecasts"]
            # Rebuild figures from raw data (figures can't be pickled)
            lstm_figs = _rebuild_lstm_figs(lstm_eval_data, monthly)
            return lstm_forecasts, lstm_eval_data, lstm_figs
        else:
            # No pkl file found — return empty
            return {}, {}, {}
    ...
    for acc in monthly['Account No'].unique():
        ...
        vals_clipped = np.clip(vals, q1 - 2*(q3-q1), q3 + 2*(q3-q1))
        scaler       = RobustScaler()
        ...
        # Store eval data — include future_pred and future_dates so pkl can rebuild figs
        lstm_eval_data[acc] = {
            "vals":         vals,
            "all_pred":     all_pred,
            "split":        split,
...
# =========================
# SAVE LSTM RESULTS TO PKL
# (only serialisable data — NO matplotlib figures)
# =========================
def save_lstm_results(lstm_forecasts, lstm_eval_data):
    """Save only raw data. Figures are rebuilt from this on cloud load."""
    data = {
        "lstm_forecasts": lstm_forecasts,
        "lstm_eval_data": lstm_eval_data,
    }
    with open(LSTM_PKL_PATH, "wb") as f:
        pickle.dump(data, f)
...
def get_lstm_evaluation(lstm_eval_data):
    ...
    lstm_metrics = {}
    for acc, data in lstm_eval_data.items():
        vals, all_pred = data["vals"], data["all_pred"]
        split, WINDOW  = data["split"], data["window"]
        ...
        lstm_metrics[acc] = {"MAE": mae, "RMSE": rmse, "Error Ratio": mae / (full_avg + 1e-6)}
...
def get_model_comparison(lstm_df, prophet_df_summary):
    if lstm_df.empty:
        out = prophet_df_summary.copy()
        out["MAE_LSTM"]          = "N/A"
        out["RMSE_LSTM"]         = "N/A"
        out["Error Ratio_LSTM"]  = "N/A"
        out["Best Model"]        = "Prophet"
        return out.rename(columns={"MAE":"MAE_Prophet","RMSE":"RMSE_Prophet","Error Ratio":"Error Ratio_Prophet"})[
            ["Account No","MAE_LSTM","RMSE_LSTM","Error Ratio_LSTM","MAE_Prophet","RMSE_Prophet","Error Ratio_Prophet","Best Model"]
        ]

    comparison_df = pd.merge(lstm_df, prophet_df_summary, on="Account No", suffixes=("_LSTM","_Prophet"))

    def select_best(row):
        try:
            diff = abs(float(row["Error Ratio_LSTM"]) - float(row["Error Ratio_Prophet"]))
            if diff < 0.01:                                             return "Both"
            elif float(row["Error Ratio_LSTM"]) < float(row["Error Ratio_Prophet"]): return "LSTM"
            else:                                                       return "Prophet"
...
def run_financial_health_module(monthly):
    ...
    for acc in monthly['Account No'].unique():
        ...
        def compute_score(row):
            sr = row['savings'] / (row['income'] + 1) if row['income'] > 0 else 0
            return (0.4 * max(0, min(1, sr)) +
                    0.3 * max(0, min(1, 1 - row['spending_ratio'])) +
                    0.3 * max(0, min(1, 1 - row['volatility'] / (row['income'] + 1)))) * 100

        def generate_recommendation(row):
            recs = []
            if row['spending_ratio'] > 0.8: recs.append("Reduce discretionary spending")
            if row['savings'] < 0:          recs.append("Expenses exceed income")
            if row['volatility'] > row['income'] * 0.5: recs.append("Spending is unstable")
            return ", ".join(recs) if recs else "Good financial health"
        ...
        health_results[acc] = (user_df[['ds','health_score','recommendations']]
                               .tail(6).rename(columns={'ds':'Month','health_score':'Health Score'}))

    health_df   = pd.concat([mh.assign(**{'Account No': acc}) for acc, mh in health_results.items()], ignore_index=True)
...
def run_insights_engine(monthly):
    ...
    for acc in monthly['Account No'].unique():
        ...
        overspend_ratio = len(user_df[user_df['expense'] > user_df['income']]) / len(user_df)
        txt.append("🚨 Frequent overspending pattern detected" if overspend_ratio > 0.5 else
                   ("⚠️ Occasional overspending observed" if overspend_ratio > 0.2 else "✅ Spending mostly within income"))

        slope = np.polyfit(np.arange(len(user_df)), user_df['savings'].values, 1)[0]
        trend = "increasing" if slope > 0 else ("decreasing" if slope < 0 else "stable")
        txt.append("📈 Savings trend is improving over time" if slope > 0 else
                   ("📉 Savings trend is declining" if slope < 0 else "➡️ Savings are stable"))

        avg_income, avg_expense = user_df['income'].mean(), user_df['expense'].mean()
        ratio = avg_expense / (avg_income + 1)
        txt.append("🚨 Almost entire income is being spent" if ratio > 0.9 else
                   ("⚠️ High spending ratio" if ratio > 0.75 else
                    ("⚠️ Moderate spending" if ratio > 0.5 else "✅ Healthy spending behavior")))

        volatility = user_df['savings'].std()
        txt.append("⚠️ Highly unstable financial pattern" if volatility > avg_income * 0.5 else
                   ("⚠️ Moderate fluctuation in savings" if volatility > avg_income * 0.2 else "✅ Stable financial behavior"))

        recent_savings = user_df.tail(3)['savings'].mean()
        txt.append("📉 Recent performance worse than usual" if recent_savings < user_df['savings'].mean()
                   else "📈 Recent performance improving")

        txt.append(f"🏆 Best month savings: ₹{user_df.loc[user_df['savings'].idxmax(),'savings']:,.0f}")
        txt.append(f"📉 Worst month savings: ₹{user_df.loc[user_df['savings'].idxmin(),'savings']:,.0f}")

        insights_results[acc] = txt
        insights_summary.append({"Account No": acc, "Overspend Ratio": round(overspend_ratio,2),
                                  "Trend": trend, "Spending Ratio": min(round(ratio,2),2),
                                  "Volatility": round(volatility,2)})

    return insights_results, pd.DataFrame(insights_summary)
...
def run_budget_module(monthly):
    ...
    for acc in monthly['Account No'].unique():
        ...
        budget_ratio      = 0.85 if spending_ratio > 0.9 else (0.75 if spending_ratio > 0.75 else 0.65)
        recommended_budget = avg_income * budget_ratio
        diff = avg_expense - recommended_budget

        if avg_expense > recommended_budget:
            status, message = "🔴 Over Budget", f"Reduce expenses by ₹{diff:,.0f}"
        elif avg_expense > recommended_budget * 0.9:
            status, message = "🟡 Near Limit", "You're close to your budget limit"
        else:
            status, message = "🟢 Within Budget", "Good financial control"

        budget_text[acc] = {"avg_income": avg_income, "avg_expense": avg_expense,
                            "recommended_budget": recommended_budget, "spending_ratio": spending_ratio,
                            "status": status, "message": message, "diff": diff}
        budget_results.append({"Account No": acc,
    ...
    return budget_text, pd.DataFrame(budget_results)
...
def run_alerts_module(monthly, df):
    ...
    for acc in monthly['Account No'].unique():
        ...
        if len(expense_df) > 0:
            ...
            top_category = cat_spend.index[0]   if len(cat_spend) > 0 else "Other"
            top_amount   = cat_spend.iloc[0]    if len(cat_spend) > 0 else 0
        else:
            top_category, top_amount = "Unknown", 0

        if ratio > 1:
            level = "HIGH"
            lines = [f"🚨 HIGH: You are overspending by ₹{avg_expense-avg_income:,.0f}/month",
                     f"💡 Major spending area: '{top_category}' (₹{top_amount:,.0f})",
                     f"👉 Action: Reduce expenses in '{top_category}' category"]
            alert_val = f"HIGH - Overspending | Category: {top_category}"
        elif ratio > 0.8:
            level = "MEDIUM"
            lines = [f"⚠️ MEDIUM: You are spending close to your income",
                     f"💡 Major spending area: '{top_category}' (₹{top_amount:,.0f})",
                     f"👉 Action: Monitor expenses in '{top_category}' category"]
            alert_val = f"MEDIUM - High spending | Category: {top_category}"
        else:
            level = "GOOD"
            lines = [f"🟢 GOOD: Your spending is under control",
                     f"💡 Highest spending category: '{top_category}' (₹{top_amount:,.0f})"]
            alert_val = f"GOOD - Stable | Category: {top_category}"

        alert_text[acc] = {"level": level, "lines": lines, "ratio": ratio,
                           "avg_income": avg_income, "avg_expense": avg_expense,
                           "top_category": top_category, "top_amount": top_amount}
        alert_results.append({"Account No": acc, "Avg Income": round(avg_income,2),
                              "Avg Expense": round(avg_expense,2), "Ratio": round(ratio,2),
                              "Top Category": top_category, "Top Spend": round(top_amount,2),
                              "Alert": alert_val})

    return alert_text, pd.DataFrame(alert_results)
...
def main_pipeline(df):
    ...
    # Save LSTM raw data locally (no figures — they can't be pickled)
    # Only save when TF was available (i.e. lstm_eval_data is non-empty)
    if lstm_eval_data:
        save_lstm_results(lstm_forecast, lstm_eval_data)
    ...
    return {
        "df": df, "monthly": monthly,
        "category_fig": fig, "category_data": category_data,
        "model_results": model_results, "best_model": best_model,
        "prophet": prophet_forecast, "prophet_figs": prophet_figs,
        "lstm": lstm_forecast, "lstm_figs": lstm_figs,
        "comparison": comparison,
        "health_results": health_results, "health": final_health,
        "insights": insights_df, "insights_text": insights_text,
        "budget": budget_df, "budget_text": budget_text,
        "alerts": alert_df, "alert_text": alert_text,
        "summary": summary
    }